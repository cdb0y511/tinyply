...
void PlyFile::PlyFileImpl::parse_data(std::istream & is, bool firstPass)
{
    ...
    auto read_list_binary = [this](const Type & t, void * dst, const size_t & stride, size_t & destOffset, std::istream & _is)
    {
        destOffset += stride;
    ...
    if (isBinary)
    {
        read = [this, &listSize, &dummyCount, &read_list_binary](PropertyLookup & f, const PlyProperty & p, uint8_t * dest, size_t & destOffset, std::istream & _is)
        {
            ...
            {
                read_list_binary(p.listType, &listSize, f.prop_stride, dummyCount, _is); // the list size
        ...
        skip = [this, &listSize, &dummyCount, &read_list_binary](PropertyLookup & f, const PlyProperty & p, std::istream & _is)
        {
            ...
            read_list_binary(p.listType, &listSize, f.list_stride, dummyCount, _is); // the list size (does not count for memory alloc)
